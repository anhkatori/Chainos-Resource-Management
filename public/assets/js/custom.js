    ...
    $(".formAddCustomer").submit(function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#pic-id")
                    .append(
                        $("<option>", { value: data.value, text: data.text })
                    )
                    .val(data.value);
    ...
    $(".load-notification").click(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                $(".wapper-noti").prepend(
                    '<li> <h1>You have <strong class="noti-number"> ' +
                        res.length +
                        " </strong> new notifications</h1></li>"
                );
                if (res.length > 0) {
                    res.map(function (item, key) {
                        $(".wapper-noti").append(
                            '<li><a href="#" id="' +
                                item.id +
                                '" class="read-notification"><span class="label label-green"></span>' +
                                item.title +
                ...
                $(".read-notification").click(function () {
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response) {
                                $(".label-transparent-black").addClass(
                                    "hidden"
                                );
                                $(".noti-number").html("0");
                ...
                $(".wapper-noti").append(
                    '<li><a href="/admin/notifications" class="text-center">Read All</a></li>'
                );
            },
        });
    });
    ...
    $(".type_attendance").on("change", function () {
        ...
        if (type == "attendance") {
            window.location.assign(
                "/admin/staffs/attendance?shop_id=" + shop_id
            );
        } else if (type == "no_attendance") {
            window.location.assign(
                "/admin/staffs/no-attendance?shop_id=" + shop_id
            );
        }
    });
